...
public class Calculadora {
	...
	public void calcular(float pago, float precio) {
		...
		this.precio = precio;
		float resto;
		aVueltas= new int[BILLETES_MONEDAS.length];

		resto = this.pago - this.precio;
		
		for(int i=0; i< BILLETES_MONEDAS.length && resto >0; i++) {
		
			
			aVueltas[i] = (int) (resto / BILLETES_MONEDAS[i]);
			resto 		= resto % BILLETES_MONEDAS[i];
			
			resto = (float)Math.round(resto*100)/100;
		}
	...
	public void imprimirVueltas() {
		...
		System.out.println("Vueltas de " + precio);
		
		for(int i=0; i<BILLETES_MONEDAS.length; i++) {
			
			if(BILLETES_MONEDAS[i] >= 5) {
				System.out.println("En " + aVueltas[i] + " Billetes de " + BILLETES_MONEDAS[i]);
			} else if(BILLETES_MONEDAS[i] >= 1) {
				System.out.println("En " + aVueltas[i] + " Monedas de " + BILLETES_MONEDAS[i] + " Euros");
			} else {
				System.out.println("En " + aVueltas[i] + " Monedas de " + BILLETES_MONEDAS[i] + " Centimos");
			}
